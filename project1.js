 var counter = 0;
    ...
    var data = myLineChart.data;

    // Create a new dataset for the next graph
    var newDataset = {
        label: 'Sample Data ' + (data.datasets.length + 1),
        data: myArray[myArray.length - 1],  // Assuming myArray contains arrays for each graph
        borderColor: 'rgba(75, 192, 192, 0.2)', // You can change the color
        borderWidth: 2
    };

    data.datasets.push(newDataset);
    myLineChart.update(); // Update the chart with the new dataset
}
...
var average = sum / myArray.length;
         ...
         function displayResult() {
          ...
          var threshold = 80;
            ...
            resultElement.classList.remove("SEVERE");
                resultElement.style.color = "green";
            } else {
